...
import kimyunjiImg from '../asset/kimyunji.jpg';
import kkapssapImg from '../asset/kkapssap.jpg';
import donmangImg from '../asset/donmang.png';
import mimjiImg from '../asset/mimji.png';
import atImg from '../asset/at.jpg';
import ioneImg from '../asset/ione.jpg';
import walloImg from '../asset/wallo.png';
import quaImg from '../asset/qua.png';
...
const LOVE_MESSAGES: string[] = [
  ...
  "이제 다음은, 로장 2세 기원!",
  "초딩커플 로장으로 합작을 하게되서 정말 행복하네요!! 열어주신 밈지님께 감사와 로장의 우당탕탕 신혼생활을 응원합니다😋",
  "서로 존중하고 신뢰하며 아름다운 가정을 이루길 바랍니다",
  "너는 '로장 합작 너무 재밌었다 다음엔 나도 꼭 참여해야지'라고 말한다!",
  `와~ 모두 마감하느라 고생 많으셨습니다~! 합작 열어주신 밈지님 메죠님 너무 감사하고 마지막까지 고생 많으셨습니다~! 웨딩이라는 주제를 보고 몇 가지 아이디어가 떠오르긴 했는데 그중에서 가장 무난한걸로 진행하게 되었네요ㅎㅎ 식상한가 싶기도하고 로장의 결혼 생활이 순탄할 것 같지도 않지만 결혼식 만큼은 그저 행복하길 바라는 씹덕의 마음으로…! 있는 힘껏 행복한 모습을 그리고 싶었어요! 화면 너머로 느껴지면 좋겠습니다!
봐주셔서 감사합니다!`,
  "맛을 아는 사람은 로장한다 로장 결혼 축하해요.",
  `사랑스러운 아기돼지 죠스케와 개잼민어른 만화가양반에게.. 결혼 진심으로 축하하고 둘이 같이 열심히 돈을 벌어서 죠스케 식비에 쓰도록 해. 
합작 즐거웠습니다 오랜만에 로장그리니 백석시인이 된 기분이고 불""로장""생 하시길 빕니다~!!`,
  "합작 같이 열어주신 메죠님 정말 감사합니다! 참가해 주신 한 분 한 분께 무진장 감사드립니다~!! 다듣 결혼 축하를 멋들어지게 해주셔서 저는 더 할 말이 없네요 ㅋㅋㅋ 이 커플, 백년해로하고 이혼후재결합하며 지지고볶고 잘 살길 바랍니다 ㅎㅎㅎ",
            ...
            <div className="w-screen h-screen flex items-center justify-center bg-wedding-cream">
              <div className="w-[80%] h-[60vh] flex gap-8 flex-col md:flex-row items-center justify-center">
                ...
                >
                  ...
                  <img src={lucaImg} className="absolute inset-0 w-full h-full object-cover opacity-60" referrerPolicy="no-referrer" />
                  <div className="relative z-10 w-full flex flex-col items-center justify-center h-full">
                    <h2 className="text-4xl font-serif text-white">Art</h2>
                  </div>
                </motion.div>
                <motion.div
                  className="novel-section w-full h-full rounded-lg shadow-lg cursor-pointer relative overflow-hidden"
                  onClick={() => navigateTo('novel')}
                  whileHover={{ scale: 1.05 }}
                  transition={{ duration: 0.3 }}
                >
                  <div className="absolute inset-0 bg-black/40"></div>
                  <img src={madisonImg} className="absolute inset-0 w-full h-full object-cover opacity-60" referrerPolicy="no-referrer" />
                  <div className="relative z-10 w-full flex flex-col items-center justify-center h-full">
                    <h2 className="text-4xl font-serif text-white">Novel</h2>
                  </div>
                </motion.div>
                <motion.div
                  className="love-section w-full h-full rounded-lg shadow-lg cursor-pointer relative overflow-hidden"
                  onClick={() => navigateTo('love')}
                  whileHover={{ scale: 1.05 }}
                  transition={{ duration: 0.3 }}
                >
                  <div className="absolute inset-0 bg-black/40"></div>
                  <img src={lauraImg} className="absolute inset-0 w-full h-full object-cover opacity-60" referrerPolicy="no-referrer" />
                  <div className="relative z-10 w-full flex flex-col items-center justify-center h-full">
